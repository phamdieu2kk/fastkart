    ...
    <div className='flex flex-col items-center justify-center w-full max-w-md p-4 m-2 mx-auto rounded bg-slate-200'>
      <img
        src={isSuccess ? SUCCESSIMAGE : FAILEDIMAGE}
         alt="Payment canceled illustration"
        width={150}
        height={150}
      ...
      {amount && <p className='mt-2 text-gray-700'>Total Amount: {displayVNDCurrency(Number(amount))}</p>}
      <Link
        to={isSuccess ? `/orders/${orderId}` : '/cart'}
        className={`p-2 px-3 mt-5 border-2 rounded font-semibold ${
          isSuccess

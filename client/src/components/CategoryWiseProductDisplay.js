...
const CategroyWiseProductDisplay = ({ category, heading }) => {
  ...
  // Tính số slide tối đa = nhỏ nhất giữa 4 và data.length
 const slidesToShow = Math.min(data.length || 4, 4);

const settings = {
  dots: false,
  infinite: true,          // bắt buộc true để autoplay hoạt động
  speed: 500,
  slidesToShow: slidesToShow,
  slidesToScroll: 1,
  nextArrow: <NextArrow />,
  prevArrow: <PrevArrow />,
  autoplay: true,           // luôn autoplay
  autoplaySpeed: 3000,      // 3 giây
  responsive: [
    { breakpoint: 1400, settings: { slidesToShow: Math.min(data.length, 4) } },
    { breakpoint: 1200, settings: { slidesToShow: Math.min(data.length, 3) } },
    { breakpoint: 1024, settings: { slidesToShow: Math.min(data.length, 3) } },
    { breakpoint: 760, settings: { slidesToShow: Math.min(data.length, 2) } },
